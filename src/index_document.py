...
def get_section_name(chunk):
    ...
    Example output: 4. PAID TIME OFF
    """
...
if __name__ == "__main__":
    ...
    # 4. Load the same embedding model used earlier.
    ...
    # 6. Connect to a persistent ChromaDB database.
    client = chromadb.PersistentClient(path=str(DATABASE_PATH))

    # 7. Create the collection if it does not already exist.
    collection = client.get_or_create_collection(name=COLLECTION_NAME)

    # 8. Store chunks, embeddings, IDs, and metadata.
    collection.upsert(
        ids=[f"chunk_{index}" for index in range(1, len(chunks) + 1)],
        documents=chunks,
    ...
    print(f"Database folder: {DATABASE_PATH}")